...
NIGHT_MAP = {
    ...
    # bottom yellow grass (day -> night)
    "#F2C94C": "#C7A13A", "#E8B62F": "#BE962B", "#D9A61B": "#B08A18", "#F6D766": "#D3B64E",
}
...
# (cat removed — the garden keeps grass, seed stalks, daisies and fireflies)
...
YELLOW_GRASS = ["#F2C94C", "#E8B62F", "#D9A61B", "#F6D766"]
...
def gen_footer(w, s):
    ...
    # yellow grass: golden blades hugging the bottom edge, front-most layer
    x = 8
    while x < 895:
        n = rnd.randint(2, 4)
        sway, beg = rnd.uniform(2.8, 5.5), rnd.uniform(0, 3)
        amp = rnd.uniform(2.5, 5)
        blades = []
        for k in range(n):
            ang = -18 + k * (36 / max(n - 1, 1)) + rnd.uniform(-5, 5)
            h = rnd.randint(12, 26)
            yc = rnd.choice(YELLOW_GRASS)
            blades.append(f'<g transform="rotate({ang:.0f})"><path d="M0 0 Q{rnd.choice([-1, 1]) * 2} {-h * 0.55:.0f} {rnd.choice([-1, 1]) * 4} {-h}" '
                          f'stroke="{yc}" stroke-width="{rnd.choice([2, 2.4, 2.8])}" fill="none" stroke-linecap="round" opacity="0.9"/></g>')
        p.append(f'<g transform="translate({x + rnd.randint(-2, 2)},140)"><g>'
                 f'<animateTransform attributeName="transform" type="rotate" values="{-amp:.1f};{amp:.1f};{-amp:.1f}" '
                 f'{SPLINE2} dur="{sway:.2f}s" begin="-{beg:.2f}s" repeatCount="indefinite"/>'
                 + "".join(blades) + '</g></g>')
        x += rnd.randint(10, 22)
    ...
    def wrap(inner):
        # headroom above the grass for seed stalks, fireflies and drifting petals
        return ('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -20 900 160" width="900" height="160">\n'
...
def parse_args(argv=None):
    ...
    p.add_argument("--out-dir", type=Path, default=Path(env("SCENE_OUT_DIR", str, "assets")),
